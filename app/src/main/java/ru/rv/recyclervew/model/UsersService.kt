package ru.rv.recyclervew.model
...
class UsersService {
    ...
    // переменная, которая будет хранить список пользователей
    private var users: MutableList<User> = mutableListOf<User>()

    //сюда будем складывать всех слушатедей, которые будут прослушивать юзеров в классе юзерсервис
    private val listeners: MutableSet<UsersListener> = mutableSetOf<UsersListener>()

    // создадим блок инит, внутри которого проинициализурем список пользователей случайным образом
    init {
    ...
    // проходит повсем элементам списка
    // it это текущий идентификатор, который сравниваем с айди пользователя, который передади в метод
    ...
    //метод перемещения пользователя по списку
